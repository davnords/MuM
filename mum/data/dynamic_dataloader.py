...
def build_dynamic_dataloader(
    ...
    Args:
        dataset_str (str): Configuration dictionary for the dataset to be instantiated.
    ...
    print(f"Randomly sampling {common_config.img_nums} frames")
    
    # Instantiate the dataset
    # dataset_obj = instantiate(datasets, common_config=common_config, image_aug=image_aug, _recursive_=False)
    dataset_obj = ComposedDataset(datasets, common_config=common_config, image_aug=image_aug)
    ...
    def __iter__(self):
        ...
        while True:
            try:
                ...
                # random_aspect_ratio = 1.0  # Fixed aspect ratio for now, can be changed later
...
class DynamicDistributedSampler(DistributedSampler):
    ...
    def __iter__(self):
        ...
        """
        while True: # Change to make it infinite
